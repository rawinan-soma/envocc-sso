...
The following were removed because Keycloak 26.x's `--import-realm` rejects them (the import is strict about `RealmRepresentation`):
...
2. **Client secrets** (e.g., `account` client): Built-in clients have blank secrets in the export; Keycloak regenerates them. For OIDC clients added in later stories, secrets will be injected via environment variable using Keycloak's Admin REST API at container startup.
...
After exporting, always strip the `components` block:
```bash
# Remove the KeyProvider components group before committing
jq 'del(.components)' keycloak/realm-export.json > /tmp/realm-stripped.json
mv /tmp/realm-stripped.json keycloak/realm-export.json
```
...
You can also run the full lint check:
```bash
...
| Setting | Value | Story |
|---------|-------|-------|
| Realm name | `envocc` | 1.2 |
| Display name | `EnvOcc SSO` | 1.2 |
| SSL Required | `external` (localhost exempt) | 1.2 |
| Locales | `en` (default), `th` | 1.2 |
| User registration | OFF | 1.2 |
| Forgot password | ON | 1.2 |
| Remember me | OFF | 1.2 |
| Email as username | ON | 1.2 |
| Login with email | ON | 1.2 |
| Access token lifespan | 900s (15 min, NFR2a) | 1.2 |
| SSO session idle | 1800s (30 min) | 1.2 |
| SSO session max | 28800s (8 h) | 1.2 |
| Brute-force protection | ON (30 failures, 15 min lockout) | 1.2 |
| Login events | ON | 1.2 |
| Admin events | ON (with details) | 1.2 |
| Event expiration | 2592000s (30 days) | 1.2 |
| Default signature algorithm | RS256 (NFR3) | 1.2 |
| CSP frame-ancestors | `'self'` | 1.2 |

OIDC clients (Rails app) are added in Story 3.1.
